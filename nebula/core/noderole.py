...
class TrainerAggregatorRoleBehavior(RoleBehavior):
    ...
    async def extended_learning_cycle(self):
        ...
        # Self update will always overwrite
        self_delta = False
        self_update_event = UpdateReceivedEvent(
            self._engine.trainer.get_model_parameters(), self._engine.trainer.get_model_weight(), self_delta, self._engine.addr, self._engine.round
        )
