...
class Engine:
    ...
    async def model_update_callback(self, source, message):
        if not message.delta:
            logging.info(f"🤖  handle_model_message | Received full model from {source} with round {message.round}")
        else:
            logging.info(f"🤖  handle_model_message | Received model delta from {source} with round {message.round}")
        ...
        updt_received_event = UpdateReceivedEvent(decoded_model, message.weight, message.delta, source, message.round)
